...
// Draws a subtle top-to-bottom gradient so the panel has depth/shine.
class GradientBackgroundView: NSView {
    override func draw(_ dirtyRect: NSRect) {
        let top    = NSColor(red: 225/255, green: 244/255, blue: 253/255, alpha: 1)
        let bottom = NSColor(red: 245/255, green: 251/255, blue: 255/255, alpha: 1)
        guard let gradient = NSGradient(starting: top, ending: bottom) else { return }
        gradient.draw(in: bounds, angle: 270)

        // Thin brand-blue accent line along the top edge.
        let accent = NSColor(red: 41/255, green: 171/255, blue: 226/255, alpha: 1)
        accent.setFill()
        NSRect(x: 0, y: bounds.maxY - 2, width: bounds.width, height: 2).fill()
    }
}

class DownloadOverlayController: NSWindowController {
    ...
    func beginTracking(filename: String) {
        ...
        statsLabel.stringValue = "Connecting…"
        statusLabel.stringValue = "Locating peers"
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
            guard let self, !self.hasProgress else { return }
            let elapsed = Date().timeIntervalSince(self.startedAt)
            self.statsLabel.stringValue = String(format: "Elapsed: %.1fs", elapsed)
        }
    }

    func update(received: Int, needed: Int) {
        guard needed > 0 else { return }

        let elapsed = Date().timeIntervalSince(startedAt)
        let percent = Double(received) / Double(needed) * 100
        ...
        statsLabel.stringValue = String(
            format: "%d / %d chunks · %.1fs", received, needed, elapsed)
    }
    ...
    func finish() {
        ...
        statusLabel.stringValue = "Opening file…"
