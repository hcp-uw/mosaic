...
func LoginKey(req protocol.LoginKeyRequest) protocol.LoginKeyResponse {
	...
	if existing, err := helpers.LoadSession(); err == nil {
		return protocol.LoginKeyResponse{
			...
			Details:         fmt.Sprintf("already logged in (key: %s...)", existing.PublicKey[:8]),
		}
	}
	...
	fmt.Printf("Daemon: logged in (key: %s...)\n", pubKeyHex[:8])

	return protocol.LoginKeyResponse{
		Success: true,
		Details: fmt.Sprintf("Logged in successfully. Your identity: %s...", pubKeyHex[:8]),
	}
}
