...
class Node:
    def __init__(self, node_id: str):
        ...
        self.bucket_sets = [set() for _ in range(160)]  # ✅ Track IDs for O(1) lookup
    ...
    def add_node(self, node: 'Node'):
        if node == self or node.id == self.id:
            return
        ...
        b_set = self.bucket_sets[bucket_index]
        
        if node.id in b_set:  # ✅ O(1) lookup
            # Move to end
    ...
    def closest_to(self, target: bytes, querier: Optional['Node'] = None):
        ...
        candidates_with_dist = [(n, distance(n.id, target)) for n in candidates]
        candidates_with_dist.sort(key=lambda x: x[1])
        return [n for n, _ in candidates_with_dist[:self.k]]
...
class DHT(MutableMapping):
    ...
    def discover(self, h: bytes):
        closest = self.node.closest_to(h)  # ✅ No self-query
        queried = set()
        queried.add(self.node.id)
        seen_ids = {n.id for n in closest}  # ✅ Track IDs
        ...
        while True:
            ...
            for peer in to_query[:self.node.k]:
                ...
                for n in new_nodes:
                    ...
                    if n.id not in seen_ids:  # ✅ O(1) lookup
                        seen_ids.add(n.id)
                        closest.append(n)
                
            closest = sorted(closest, key=lambda n: distance(n.id, h))[:self.node.k]
            seen_ids = {n.id for n in closest}  # Update after trimming
